...
| 增加聯絡 / 關於頁文字 | 後台 → 站點設定 → 聯絡資訊（含地址、LINE QR）/ about_html；地址地圖見 `src/components/site/address-map.tsx`；LINE QR 存 `items` bucket 路徑 `site/line-qr.*`（見 `src/lib/line-qr-storage.ts`） |
